...
ログイン画面 (`LoginView`) の入口で **「はじめる (新規会員登録)」と「ログイン (データ引き継ぎ)」** に分岐する:

```
起動 → 利用規約 → [未サインインなら] ログイン画面
  ├ はじめる (会員登録) → SMS認証 → ローカル状態を初期化 (AccountLocalState)
  │    → プロフィール入力 → オンボーディング → ファッションタイプ診断
  │    → スタンダードアイテム選択 → ホーム
  └ ログイン → SMS認証 → 端末のデータを引き継いで続きから再開 (通常はホーム直行)
```

- **会員登録**: 認証成功後に `AccountLocalState.resetForNewRegistration()`(`Core/Auth/AccountLocalState.swift`)が
  アカウント紐付きの UserDefaults を全消去 (利用規約同意と通知プリプロンプト記録のみ保持) し、全オンボーディング導線を通す。
  端末に利用データが残っている場合は破壊的操作の確認ダイアログを挟む (「引き継いでログイン」への逃げ道つき)。
- **ログイン**: **UserDefaults の `userId` は変更しない**ため、既存データ (クローゼット・コーデ等) はそのまま。
  認証必須化前からの既存ユーザーはこちらを選べばデータを引き継いでホームへ直行する。
- ログアウトは Firebase のサインアウトのみで、UserDefaults は消さない。
  同じ電話番号で「ログイン」すれば同じ Firebase UID に戻り、データも引き継がれる。
...
1. 新規 (まっさらな端末): アプリ削除 → 起動 → 規約 → 「はじめる」 → 認証 → プロフィール入力 → オンボ → 診断 → ホーム
2. 会員登録のやり直し (データありの端末): 「はじめる」 → 確認ダイアログ → 「新しく始める」 → 認証 → **プロフィール入力からの全導線が再度出る**こと
3. 既存ユーザーの引き継ぎ: 「ログイン」 → 認証 → プロフィール入力を**スキップして**ホーム、`userId` (UserDefaults) が変わらないこと
4. ログアウト → 「ログイン」で再認証 → データがそのまま残っていること
